//使用方法 直接require這個 package就好
exports.axiosJs = require('./axios');
exports.bullJs = require('./bull');
exports.cacheJs = require('./cache');
exports.cheerioJs = require('./cheerio');
exports.dayJs = require('./day');
exports.encryptJs = require('./encrypt');//加密用
exports.faststringifyJs = require('./faststringify');//跟stringify功能一樣 優化過效能更好
exports.fsextraJs = require('./fsextra');
exports.logJs = require('./log');
exports.lrucacheJs = require('./lrucache');
exports.mathJs = require('./math');
exports.momentJs = require('./moment');//處理日期時間用
exports.nodemailerJs = require('./nodemailer');//mail用
exports.osJs = require('./os');//系統
exports.qrcodeJs = require('./qrcode');
exports.smsJs = require('./sms');//簡訊
exports.lodashJs = require('./lodash');//計算用
exports.validatorJs = require('./validator');//驗證用
exports.numeralJs = require('./numeral');//數字用

